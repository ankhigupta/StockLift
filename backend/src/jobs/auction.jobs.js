        ...
        if (!auction.highest_bidder_id) {
          ...
          await client.query(
            `UPDATE bids SET status = 'OUTBID' 
             ...
             AND bid_amount = (SELECT MAX(bid_amount) FROM bids WHERE auction_id = $1 AND bidder_id != $2)`,
            [auction.id, auction.highest_bidder_id]
          );
          await client.query(
            `UPDATE bids SET status = 'LOST' WHERE auction_id = $1 AND status NOT IN ('WON', 'LOST', 'OUTBID')`,
            [auction.id]
          );
...
const promoteUnpaidOrders = async () => {
  ...
  try {
    const unpaidOrders = await client.query(
      `SELECT * FROM orders WHERE status = 'PENDING' AND payment_deadline <= NOW()`
    );
    for (const order of unpaidOrders.rows) {
      await client.query("BEGIN");
      try {
        await client.query(
          `INSERT INTO strikes (buyer_id, auction_id, reason) VALUES ($1, $2, 'Payment not completed within 24 hours')`,
        ...
        const secondBidder = await client.query(
          `SELECT * FROM bids WHERE auction_id = $1 AND bidder_id != $2 AND status = 'OUTBID'
        ...
        if (secondBidder.rows.length === 0) {
          ...
          await client.query("UPDATE auctions SET status = 'EXPIRED', updated_at = NOW() WHERE id = $1", [order.auction_id]);
          ...
          await client.query("UPDATE orders SET status = 'PROMOTED', updated_at = NOW() WHERE id = $1", [order.id]);
          await client.query(
            `INSERT INTO orders (auction_id, buyer_id, seller_id, final_amount, payment_deadline) VALUES ($1, $2, $3, $4, $5)`,
            [order.auction_id, newBuyer.bidder_id, order.seller_id, newBuyer.bid_amount, newDeadline]
          );
          await client.query("UPDATE bids SET status = 'WON' WHERE id = $1", [newBuyer.id]);
...
const initCronJobs = () => {
  ...
  cron.schedule("*/5 * * * *", () => { promoteUnpaidOrders(); });
